...
async function createMemoryDir(root: string): Promise<string> {
  const dir = join(root, '.cyrene', 'memory')
  await mkdir(dir, { recursive: true })
  return dir
}

async function createSessionsDir(root: string): Promise<string> {
  const dir = join(root, '.cyrene', 'memory', 'sessions')
  await mkdir(dir, { recursive: true })
  return dir
}
...
describe('loadRuleStack', () => {
  ...
  it('does not traverse project rules when cwd is outside the global home root', async () => {
    ...
    const userCyreneDir = join(home, '.cyrene')
    await mkdir(userCyreneDir, { recursive: true })
    await mkdir(join(outside, '.cyrene'), { recursive: true })
    await writeFile(join(userCyreneDir, 'Rule.md'), 'Global rule.\n')
    await writeFile(join(outside, '.cyrene', 'Rule.md'), 'Outside rule.\n')

    await expect(loadRuleStack(outside, userCyreneDir)).resolves.toBe(
...
describe('loadMemories', () => {
  ...
  it('returns an empty string when .cyrene/memory/ does not exist', async () => {
    const root = await createTempDir()

    await expect(loadMemories(root)).resolves.toBe('')
  })

  it('returns an empty string when .cyrene/memory is a symlink outside the project', async () => {
    const root = await createTempDir()
    ...
    await symlink(outsideMemoryDir, join(root, '.cyrene', 'memory'))
  ...
  it('skips memory index entries whose paths escape .cyrene/memory', async () => {
    const root = await createTempDir()
    const memoryDir = await createMemoryDir(root)
    await writeFile(
      join(memoryDir, 'MEMORY.md'),
  ...
  it('skips symlinked memory files that resolve outside .cyrene/memory', async () => {
    const root = await createTempDir()
    const memoryDir = await createMemoryDir(root)
    await writeFile(
      join(memoryDir, 'MEMORY.md'),
...
describe('loadDaily', () => {
  ...
  it('returns an empty string when .cyrene is a symlink outside the project', async () => {
    const root = await createTempDir()
    const outsideCyrene = await createTempDir()
    await mkdir(join(outsideCyrene, 'memory'), { recursive: true })
    await writeFile(join(outsideCyrene, 'memory', 'daily.md'), 'outside\n')
    await symlink(outsideCyrene, join(root, '.cyrene'))
...
describe('scoped memory loading', () => {
  ...
  it('loads project and global memories through separate APIs', async () => {
    ...
    const userCyreneDir = join(await createTempDir(), '.cyrene')
    const globalMemoryDir = join(userCyreneDir, 'memory')
    ...
    await expect(loadGlobalMemories(userCyreneDir)).resolves.toBe('## Global Memory: Global\n\nGlobal content.')
  })

  it('does not load project memories through a symlinked .cyrene directory', async () => {
    const root = await createTempDir()
    const outsideCyrene = await createTempDir()
    await mkdir(join(outsideCyrene, 'memory'), { recursive: true })
    await writeFile(join(outsideCyrene, 'memory', 'MEMORY.md'), '- [Outside](outside.md) — outside\n')
    await writeFile(join(outsideCyrene, 'memory', 'outside.md'), 'Outside content.\n')
    await symlink(outsideCyrene, join(root, '.cyrene'))
  ...
  it('returns empty string for missing global memory scope', async () => {
    ...
    await expect(loadGlobalMemories(userCyreneDir)).resolves.toBe('')
  })
})
...
describe('loadRecentSummaries', () => {
  ...
  it('returns an empty string when .cyrene/memory/sessions does not exist', async () => {
    const root = await createTempDir()
  ...
  it('skips symlinked session files that resolve outside .cyrene/memory/sessions', async () => {
    const root = await createTempDir()
    const sessionsDir = await createSessionsDir(root)
...
describe('saveSessionSummary', () => {
  ...
  it('writes session summary to a date-named file, creating directories as needed', async () => {
    ...
    const sessionsDir = join(root, '.cyrene', 'memory', 'sessions')
  ...
  it('does not write session summary through a .cyrene/memory symlink outside the project', async () => {
    const root = await createTempDir()
    const outsideMemoryDir = await createTempDir()
    await mkdir(join(root, '.cyrene'), { recursive: true })
    await symlink(outsideMemoryDir, join(root, '.cyrene', 'memory'))
  ...
  it('does not create session summary through a .cyrene symlink outside the project', async () => {
    const root = await createTempDir()
    const outsideCyreneDir = await createTempDir()
    await symlink(outsideCyreneDir, join(root, '.cyrene'))
    ...
    await expect(readdir(join(outsideCyreneDir, 'memory'))).rejects.toMatchObject({ code: 'ENOENT' })
    expect(error).toBeInstanceOf(Error)
  })
...
describe('updateMemoryIndex', () => {
  ...
  it('creates MEMORY.md with entry when it does not exist, creating directories as needed', async () => {
    ...
    await expect(readFile(join(root, '.cyrene', 'memory', 'MEMORY.md'), 'utf8')).resolves.toBe(
      '- [Architecture](architecture.md) — agent loop notes\n'
    )
  })
  ...
  it('does not update memory index through a .cyrene/memory symlink outside the project', async () => {
    const root = await createTempDir()
    const outsideMemoryDir = await createTempDir()
    await mkdir(join(root, '.cyrene'), { recursive: true })
    await symlink(outsideMemoryDir, join(root, '.cyrene', 'memory'))
  ...
  it('does not create memory index through a .cyrene symlink outside the project', async () => {
    const root = await createTempDir()
    const outsideCyreneDir = await createTempDir()
    await symlink(outsideCyreneDir, join(root, '.cyrene'))
...
describe('writeMemoryEntry', () => {
  ...
  it('creates a memory file and appends the index entry', async () => {
    ...
    await expect(readFile(join(root, '.cyrene', 'memory', 'architecture.md'), 'utf8')).resolves.toBe(
      'Use small context windows.\n'
    )
    await expect(readFile(join(root, '.cyrene', 'memory', 'MEMORY.md'), 'utf8')).resolves.toBe(
      '- [Architecture](architecture.md) — agent loop notes\n'
  ...
  it('rejects summaries longer than the configured maximum without truncating', async () => {
    ...
    await expect(readFile(join(root, '.cyrene', 'memory', 'long.md'), 'utf8')).rejects.toMatchObject({
      code: 'ENOENT'
    })
  })
  ...
  it('rejects invalid memory entry types without writing files', async () => {
    ...
    await expect(readFile(join(root, '.cyrene', 'memory', 'bad.md'), 'utf8')).rejects.toMatchObject({
      code: 'ENOENT'
    })
  })
