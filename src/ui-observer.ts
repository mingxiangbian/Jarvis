...
export function renderWelcome(input: { modelName: string; color?: boolean }): string {
  ...
  const title = color
    ? `${chalk.hex(PRISM_THEME.colors.iceCyan)('Cyrene')} ${chalk.hex(PRISM_THEME.colors.lavender)('·')} ${chalk.hex(PRISM_THEME.colors.softPink)('Prism Agent')}`
    : 'Cyrene · Prism Agent'
...
export function createTerminalObserver(
  ...
  options: { color?: boolean; spinner?: boolean; responseDivider?: boolean } = {}
): AgentObserver {
  const color = options.color ?? true
  const spinnerEnabled = options.spinner ?? true
  const responseDivider = options.responseDivider ?? true
